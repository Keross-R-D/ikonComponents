...
export const ResourceTable: React.FC<ResourceTableProps> = ({ resourceDataWithAllocation, userMaps, monthsRange }) => {
        ...
        <Card className="p-0 bg-background">
            <CardContent className="p-0">
                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableHead className="w-[300px] py-5">Task Name</TableHead>
                            <TableHead className="w-[200px] py-5">Employee Name</TableHead>
                            <TableHead className="w-[200px] py-5">Role</TableHead>
                            {monthsRange.map((month) => (
                                <TableHead key={month} className="w-[100px] py-5">
                                    {month.replace("_", " ")}
                                </TableHead>
                            ))}
                        </TableRow>
                                    ...
                                    <TableRow key={`${resource.taskId}-${resource.resourceId}`} className="hover:bg-muted/30">
                                        ...
                                        <TableCell>
                                            <Select defaultValue={resource.resourceId}>
                                                <SelectTrigger className="w-full bg-secondary dark:bg-secondary">
